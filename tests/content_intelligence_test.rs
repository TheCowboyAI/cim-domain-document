...
use cim_domain_document::{commands::*, services::*, value_objects::*};
use uuid::Uuid;
...
fn test_entity_extraction_workflow() {
    ...
    let entities = service
        .extract_entities(content, &extract_cmd.options)
        .unwrap();
    ...
    assert!(person_entities
        .iter()
        .any(|e| e.text.contains("John Smith")));
    ...
    assert!(org_entities
        .iter()
        .any(|e| e.text.contains("Inc.") || e.text.contains("LLC")));

    // Check for concepts
    let concept_entities: Vec<_> = entities
        .iter()
...
fn test_summarization_workflow() {
    ...
    for length in lengths {
        ...
        let summary = service
            .generate_summary(content, &summary_cmd.length, "en")
            .unwrap();
...
fn test_document_classification_workflow() {
    ...
    let tech_classifications = service
        .classify_document(tech_content, &DocumentType::Report)
        .unwrap();
    ...
    let business_classifications = service
        .classify_document(business_content, &DocumentType::Report)
        .unwrap();
    assert!(!business_classifications.is_empty());
    assert_eq!(business_classifications[0].category, "Business");
    assert!(business_classifications[0]
        .labels
    ...
    let legal_classifications = service
        .classify_document(legal_content, &DocumentType::Contract)
        .unwrap();
    assert!(!legal_classifications.is_empty());
    assert_eq!(legal_classifications[0].category, "Legal");
    assert!(legal_classifications[0]
        .labels
...
fn test_extraction_options_customization() {
    ...
    assert!(entities
        .iter()
        .all(|e| matches!(e.entity_type, EntityType::Person | EntityType::Organization)));

    // Should not have concepts
    assert!(!entities
        .iter()
        .any(|e| matches!(e.entity_type, EntityType::Concept)));
...
fn test_summary_quality_metrics() {
    ...
    let summary = service
        .generate_summary(content, &SummaryLength::Standard, "en")
        .unwrap();
...
fn test_multi_category_classification() {
    ...
    let classifications = service
        .classify_document(mixed_content, &DocumentType::Report)
        .unwrap();
    ...
    let categories: Vec<&str> = classifications
        .iter()
